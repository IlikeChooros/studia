public class Main {
    ...
    public static Figures.FigureTypeOneParam getFigureTypeOneParam(AllFigureTypes type) {
        switch (type) {
            case OKRAG:
    ...
    public static Figures.FigureTypeTwoParam getFigureTypeTwoParam(AllFigureTypes type) {
        switch (type) {
            case PROSTOKAT:
                return Figures.FigureTypeTwoParam.RECTANGLE;
            case ROMB:
            default:
                return Figures.FigureTypeTwoParam.RHOMBUS;
        }
    }
    ...
    public static int parseArgs(String[] args, int start) {
        ...
        if (isOneParamTypeFigure(alltype)) {
            Figures.FigureTypeOneParam figureType = getFigureTypeOneParam(alltype);
            System.out.println(
                figureType.getName() + " --> " +
                " pole: " + figureType.getArea(params[0]) +
                " obwód: " + figureType.getPerimeter(params[0])
            );
        } else {
            Figures.FigureTypeTwoParam figureType = getFigureTypeTwoParam(alltype);
            double param2 = params[1];

            if (alltype == AllFigureTypes.ROMB) {
                param2 = params[4];
            }

            System.out.println(
                figureType.getName() + " --> " +
                " pole: " + figureType.getArea(params[0], param2) +
                " obwód: " + figureType.getPerimeter(params[0], param2)
            );
        }
